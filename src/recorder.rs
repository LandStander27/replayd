...
impl Recorder {
	...
	pub fn start(&mut self, settings: &Settings) -> Result<()> {
		...
		let process = Command::new("gpu-screen-recorder")
			.args([
				"-s",
				settings.resolution.get_str("cmd").unwrap(),
				"-w",
				settings.display.as_ref().context("no display specified")?,
				"-v",
				"no",
				"-f",
				settings.frame_rate.get_str("cmd").unwrap(),
				"-r",
				&settings.buffer_length.to_string(),
				"-k",
				settings.codec.get_str("cmd").unwrap(),
				"-c",
				settings.container.get_str("cmd").unwrap(),
				"-a",
				"default_output|default_input",
				"-q",
				settings.quality.get_str("cmd").unwrap(),
				"-bm",
				"qp",
				"-encoder",
				"gpu",
